...
import { 
    ...
    Gamepad2, Eye, EyeOff, Ticket, Sparkles, CheckSquare, Square
} from 'lucide-react';
...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin, onRegister }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isLogin && !agreedToPolicy) {
        setErrorMsg("يجب الموافقة على سياسة الخصوصية وشروط الاستخدام للمتابعة.");
        return;
    }
    ...
    try {
      ...
      // تخصيص رسائل الخطأ كما طلب المستخدم
      if (msg.includes("Invalid login credentials") || msg.includes("Email not confirmed")) {
          msg = "معلومات الدخول خاطئة، أو الحساب غير موجود (تأكد من إنشائه أولاً)";
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 bg-[#020202] font-cairo overflow-y-auto overflow-x-hidden custom-scrollbar touch-pan-y">
      <PrivacyModal isOpen={showPrivacy} onClose={() => { setShowPrivacy(false); if(!isLogin) setAgreedToPolicy(true); }} />
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5 relative z-10">
                    ...
                    {!isLogin && (
                        <>
                            ...
                            <div className="flex items-center gap-3 bg-black/40 p-3 rounded-xl border border-white/5 mt-2 animate-slideIn cursor-pointer" onClick={() => setAgreedToPolicy(!agreedToPolicy)}>
                                <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${agreedToPolicy ? 'bg-brand border-brand' : 'border-gray-600'}`}>
                                    {agreedToPolicy && <CheckSquare size={14} className="text-black" />}
                                </div>
                                <label className="text-[10px] font-bold text-gray-400 cursor-pointer select-none">
                                    قرأت وأوافق على <span className="text-brand underline" onClick={(e) => { e.stopPropagation(); setShowPrivacy(true); }}>سياسة الخصوصية وشروط الاستخدام</span>
                                </label>
                            </div>
                        </>
                    )}
