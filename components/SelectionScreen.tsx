...
const SelectionScreen: React.FC<SelectionScreenProps> = ({ onStartGame, onBack }) => {
  ...
  const handleAiGenerate = async () => {
      ...
      if (!contentText) {
          window.addToast("لا يوجد محتوى كافٍ في هذا القسم لتوليد الأسئلة", "info");
          return;
      }
      ...
      المحتوى المرجعي: ${contentText.substring(0, 30000)}`; // Increased limit to 30k chars

      // Initialize AI inside try block to catch init errors
      const ai = initGemini();
      ...
      // تنظيف الرد من علامات Markdown لضمان صحة JSON
      const rawText = response.text || '[]';
      const cleanJson = rawText.replace(/```json/g, '').replace(/```/g, '').trim();
      
      let generatedQs;
      try {
          generatedQs = JSON.parse(cleanJson);
      } catch (jsonError) {
          throw new Error("فشل في معالجة رد الذكاء الاصطناعي (JSON Error)");
      }
      
      if (!Array.isArray(generatedQs) || generatedQs.length === 0) {
          throw new Error("لم يتمكن الذكاء الاصطناعي من توليد أسئلة صالحة من هذا المحتوى");
      }

      const lessonTitle = selectedLessonId 
          ? (availableLessons.find(l => l.id === selectedLessonId)?.title || selectedTrimester) 
          : selectedTrimester;
      ...
      const finalQs: Question[] = generatedQs.map((q: any, idx: number) => ({
          ...
          chapter: selectedSection,
          lesson: lessonTitle,
          isAi: true
      }));

      if (finalQs.length < 5) {
          window.addToast("لم نتمكن من توليد أسئلة كافية من هذا المحتوى", "error");
          return;
      }
