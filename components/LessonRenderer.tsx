...
const LessonRenderer: React.FC<LessonRendererProps> = ({ lessonId, content, subjectId, sectionId }) => {
    ...
    const handleSpeak = async (text: string, lang: 'french' | 'english') => {
        ...
        const cleanText = text.replace(/[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/g, '').trim();
        ...
        try {
            const ai = initGemini();
            // استخدام "Kore" للإنجليزية لأنه أسرع وأكثر استقراراً من "Puck"
            const voiceName = lang === 'french' ? 'Charon' : 'Kore'; 
            
            // تبسيط الأمر للنموذج الإنجليزي لتسريع الاستجابة
            const prompt = lang === 'french' 
                ? `Act as a French TTS engine. Pronounce precisely in Standard French (IPA): "${cleanText}"`
                : `Act as a clear US English TTS. Speak clearly and slowly: "${cleanText}"`;
            ...
            if (base64Audio) {
                ...
                // مهم جداً: استئناف السياق الصوتي إذا كان معلقاً (يحدث في المتصفحات الحديثة)
            ...
            <div className="animate-fadeIn w-full overflow-x-hidden pb-40 px-2" dir="rtl">
                <div style={containerStyle} className="space-y-8">
                    {data.parts.map((part, idx) => (
                        <div key={idx} className="bg-neutral-900/40 p-6 rounded-[2.5rem] border border-white/5 shadow-2xl relative overflow-hidden transform-gpu">
        ...
        return (
            <div className="animate-fadeIn w-full overflow-x-hidden pb-40" dir="rtl">
                <div style={containerStyle} className="space-y-4">
                    ...
                    <div className="bg-neutral-900/80 p-5 rounded-2xl border border-brand/20 relative mx-1 transform-gpu">
                    ...
                    {(data.positions || []).map((pos, pIdx) => (
                        <div key={pIdx} className="space-y-4">
                            ...
                            {(pos.theories?.[0]?.philosophers || []).map((philo, phIdx) => (
                                <div key={phIdx} className="bg-neutral-900/40 border border-white/5 rounded-3xl p-5 space-y-4 shadow-lg mx-1 transform-gpu">
                                    ...
                                    <button onClick={() => handleExplain(philo)} className="w-full py-3 bg-white/5 rounded-xl text-[9px] font-black uppercase flex items-center justify-center gap-2 active:scale-95 transition-transform">
                            ...
                            <div className="bg-red-500/5 p-5 rounded-2xl border border-red-500/20 mx-1 transform-gpu">
                    ...
                    <div className="bg-neutral-900/60 p-5 rounded-2xl border border-purple-500/20 mx-1 transform-gpu">
                    ...
                    <div className="bg-neutral-900/90 p-6 rounded-3xl border border-green-500/30 mx-1 shadow-2xl transform-gpu">
        ...
        <div className="animate-fadeIn w-full overflow-x-hidden pb-40 px-2" dir={direction}>
            <div style={containerStyle} className="space-y-6">
                ...
                {blocks.map((block, idx) => (
                    <div key={idx} className="mx-1 transform-gpu">
                        ...
                        {block.type === 'term_entry' && (
                            <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-[2rem] mb-4 shadow-xl flex flex-col gap-3 transform-gpu">
                                <div className={`flex items-center justify-between gap-3 ${isForeignLang ? 'flex-row' : 'flex-row-reverse'}`}>
                                    ...
                                    {(subjectId === 'french' || subjectId === 'english') && (
                                        <button onClick={() => handleSpeak(block.text, subjectId as any)} className="p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-all text-brand active:scale-90">
                                            {isSpeaking === block.text ? <Loader2 className="animate-spin" size={20}/> : <Volume2 size={20} />}
                                        </button>
                                    )}
                                </div>
