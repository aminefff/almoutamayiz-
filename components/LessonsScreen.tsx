...
import { CacheManager } from '../lib/cache'; // Import Cache
...
const GLASS_CARD = "bg-neutral-900/50 backdrop-blur-2xl border border-white/5 shadow-2xl hover:border-brand/30 active:scale-[0.97] transition-all duration-300 cursor-pointer overflow-hidden relative group clickable gpu-accelerated";
...
const LessonsScreen: React.FC<LessonsScreenProps> = ({ user, onUpdateUserScore }) => {
    ...
    // Load progress from cache on mount
    useEffect(() => {
        // Force refresh progress if needed? No, prefetched in App.tsx
    }, []);
    ...
    const toggleLessonCompletion = async (e: React.MouseEvent, lessonId: number) => {
        ...
        // Optimistic UI Update using Cache
        CacheManager.toggleCompletion(lessonId);
        
        // Force re-render trick or just let the cache handle logic, but we need React state update to show visual change
        ...
        try {
            ...
            // Revert on error
            CacheManager.toggleCompletion(lessonId); // Revert cache
    ...
    const fetchLessons = async () => {
        ...
        // Check Cache First
        const cached = CacheManager.getLessons(sectionId);
        if (cached) {
            setLessons(cached);
            return;
        }
        ...
        try {
            ...
            if (data) {
                setLessons(data);
                CacheManager.setLessons(sectionId, data);
            }
    ...
    const fetchExams = async () => {
        ...
        if (subjectName) {
            // Check Cache
            const cached = CacheManager.getExams(subjectName);
            if (cached) {
                setExams(cached);
                return;
            }
            ...
            if (data) {
                setExams(data);
                CacheManager.setExams(subjectName, data);
            }
            setLoading(false);
        }
    };
    ...
    return (
        <div className="h-full flex flex-col custom-scrollbar overflow-y-auto pb-32 optimize-scrolling">
            <div key={step} className="section-entry px-6">
                    ...
                    <div className="max-w-2xl mx-auto w-full pt-4">
                        ...
                        <div className="grid gap-3">
                            {loading ? <div className="py-20 text-center"><Loader2 className="animate-spin mx-auto text-brand" size={40} /></div> : lessons.map((lesson, idx) => {
                                const isDone = CacheManager.hasCompleted(lesson.id); // Sync with cache for speed
                                return (
                                    <div key={lesson.id} onClick={() => { setActiveLesson(lesson); setStep('lesson_content'); playClickSound(); }} className={`bg-neutral-900/40 p-6 rounded-3xl border border-white/5 flex items-center gap-5 hover:border-brand/40 transition-all cursor-pointer group clickable gpu-accelerated ${isDone ? 'opacity-80' : ''}`}>
                ...
                {step === 'exams_hub' && (
                    <div className="max-w-2xl mx-auto w-full pt-4">
                        ...
                        <div className="grid gap-3">
                            {exams.map((exam) => (
                                <a key={exam.id} href={exam.pdf_url} target="_blank" rel="noreferrer" className="bg-neutral-900/40 p-6 rounded-[2rem] border border-white/5 flex items-center justify-between hover:border-brand/40 transition-all group clickable gpu-accelerated">
