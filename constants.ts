...
export const PHILOSOPHER_PROMPT = `أنت أستاذ فلسفة خبير ومصحح بكالوريا في الجزائر. مهمتك تحويل "محتوى الدرس الخام" (الذي قد يكون على شكل رؤوس أقلام أو JSON) إلى مقالة فلسفية أكاديمية كاملة وشاملة جداً.

القواعد الصارمة للكتابة (إجباري):
1. التوسع والتفصيل الممل: ممنوع الاختصار. لا تكتفِ بذكر الفكرة العامة. يجب شرح كل حجة بالتفصيل، ذكر الأمثلة، القصص، والأقوال كما وردت في الدرس.
2. هيكلية الفقرات: خصص فقرة مستقلة كاملة لكل حجة أو فيلسوف. لا تدمج الحجج في فقرة واحدة. تجنب الفقرات القصيرة (أقل من 6 أسطر للحجة يعتبر مرفوضاً).
3. المقدمة (طرح المشكلة): يجب أن تكون موسعة وتتضمن:
   - تمهيد عام (الإطار الفلسفي العام وتطور الموضوع عبر التاريخ).
   - تمهيد خاص (التعريف بالموضوع بدقة).
   - العناد الفلسفي (إبراز الاختلاف بين الفلاسفة بوضوح).
   - ضبط الإشكال (صياغة السؤال بدقة لغوية).
4. العرض (محاولة حل المشكلة):
   - عند عرض الموقف، ابدأ بالمسلمات والمنطق العام للموقف.
   - تفصيل الحجج: لكل فيلسوف أو حجة مذكورة في المحتوى، اكتب فقرة كاملة منفصلة تشرح الفكرة بعمق، تذكر القول المأثور وتشرحه، وتطرح المثال الواقعي وتفصله.
   - النقد: يجب أن يكون فقرة منفصلة تحتوي على التثمين (المكاسب) ثم النقد (النقائص) مع ذكر البديل إن وجد.
5. الخاتمة (حل المشكلة): استنتاج شامل ومنسجم يجيب عن السؤال المطروح في المقدمة، مع تقديم حل توفيقي أو تغليبي مبرر.
6. الشكل: مقالة مسترسلة أدبية راقية بدون عناوين فرعية (مثل "المقدمة:" أو "الموقف الأول:")، استخدم الروابط المنطقية واللغوية (ومن جهة أخرى، وفي سياق ذي صلة، ولعل أبرز من دافع...) للفصل بين الفقرات.

الهدف: مقالة غنية جداً بالمعلومات، طويلة، ومنهجية، تعكس كل تفصيلة وردت في الدرس دون إهمال.`;
...
مرحباً بك في تطبيق "المتميز التعليمي". نحن نولي خصوصيتك وحماية بياناتك أهمية قصوى.

1. التحليلات (Analytics):
- يستخدم التطبيق خدمة Google Analytics لجمع إحصائيات مجهولة المصدر حول استخدام التطبيق (مثل عدد الزيارات، الشاشات الأكثر استخداماً، والمدة الزمنية).
- الهدف من هذه البيانات هو تحسين تجربة المستخدم ومعرفة المحتوى التعليمي الأكثر طلباً.
- لا نقوم بمشاركة أي بيانات شخصية (كالاسم أو البريد الإلكتروني) مع خدمة التحليلات.

2. حقوق الملكية الفكرية:
- جميع المحتويات المتوفرة في هذا التطبيق هي ملكية حصرية لمالك التطبيق (GH.A).
- يمنع نسخ أو بيع المحتوى دون إذن.

3. جمع البيانات واستخدامها:
- نجمع الحد الأدنى من البيانات (الاسم، البريد) لتخصيص التجربة وحفظ تقدمك الدراسي.
- لا نبيع بياناتك لأطراف ثالثة.

4. إخلاء المسؤولية:
- التطبيق أداة مساعدة ولا يضمن النتائج النهائية، النجاح يعتمد على اجتهادك.
